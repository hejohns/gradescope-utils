...
if(!defined $options{fun}){
    carp '[error] `-f` required-- see `--help`';
...
capture_stdout {
    run ['cp', $submissions_zip, $tmpdir] or confess;
};
capture_stdout {
    run ['unzip', '-d', $tmpdir, File::Spec->catfile($tmpdir, basename($submissions_zip))] or confess "'${\(File::Spec->abs2rel($submissions_zip))}' is probably not a zip";
};
...
join.pl : B<zip → [token2uniqname, submissions]>
